import type { Meta, StoryObj } from '@storybook/react';

import type { ShortBlogPostsGetVariables } from '@entities/blog-posts';

import type { BlogPostsPageViewProps } from './BlogPostsPage';
import { BlogPostsPageView } from './BlogPostsPage';

const mockBlogPosts = [
  {
    alternatives: [],
    author: { id: 'author-1', name: 'Jane Doe' },
    createdAt: '2024-07-14T10:00:00.000Z',
    entryId: 'entry-1',
    id: '1',
    languageCode: 'en',
    publishedAt: '2024-07-14T10:00:00.000Z',
    readingTime: 5,
    shortDescription: 'A deep dive into React Server Components.',
    slug: 'react-server-components',
    tags: [
      { id: 'tag-1', name: 'React', slug: 'react' },
      { id: 'tag-2', name: 'Server Components', slug: 'server-components' },
    ],
    title: 'React Server Components Explained',
    type: 'blog_post',
  },
  {
    alternatives: [],
    author: { id: 'author-2', name: 'John Smith' },
    createdAt: '2024-07-13T09:00:00.000Z',
    entryId: 'entry-2',
    id: '2',
    languageCode: 'en',
    publishedAt: '2024-07-13T09:00:00.000Z',
    readingTime: 3,
    shortDescription: 'Understanding TypeScript generics with real-world examples.',
    slug: 'typescript-generics',
    tags: [
      { id: 'tag-3', name: 'TypeScript', slug: 'typescript' },
      { id: 'tag-4', name: 'Generics', slug: 'generics' },
    ],
    title: 'Mastering TypeScript Generics',
    type: 'blog_post',
  },
];

const mockBreadcrumbs = [
  { label: 'Home', to: '/' },
  { label: 'Blog', to: '/blog' },
];

const mockBlogPostsVars: ShortBlogPostsGetVariables = { locale: 'en', sort: '-createdAt' };
const noopRef = () => {};

const meta: Meta<typeof BlogPostsPageView> = {
  component: BlogPostsPageView,
  tags: ['autodocs'],
  title: 'Blog/BlogPostsPage',
};
export default meta;

type Story = StoryObj<typeof BlogPostsPageView>;

export const Default: Story = {
  args: {
    blogPosts: mockBlogPosts,
    blogPostsVars: mockBlogPostsVars,
    breadcrumbs: mockBreadcrumbs,
    error: undefined,
    hasNextPage: true,
    isError: false,
    isFetchingNextPage: false,
    isPending: false,
    onRetry: () => {},
    ref: noopRef,
  } satisfies BlogPostsPageViewProps,
};

export const Empty: Story = {
  args: {
    blogPosts: [],
    blogPostsVars: mockBlogPostsVars,
    breadcrumbs: mockBreadcrumbs,
    error: undefined,
    hasNextPage: false,
    isError: false,
    isFetchingNextPage: false,
    isPending: false,
    onRetry: () => {},
    ref: noopRef,
  } satisfies BlogPostsPageViewProps,
};

export const Pending: Story = {
  args: {
    blogPosts: [],
    blogPostsVars: mockBlogPostsVars,
    breadcrumbs: mockBreadcrumbs,
    error: undefined,
    hasNextPage: false,
    isError: false,
    isFetchingNextPage: false,
    isPending: true,
    onRetry: () => {},
    ref: noopRef,
  } satisfies BlogPostsPageViewProps,
};

export const ErrorState: Story = {
  args: {
    blogPosts: [],
    blogPostsVars: mockBlogPostsVars,
    breadcrumbs: mockBreadcrumbs,
    error: 'Something went wrong',
    hasNextPage: false,
    isError: true,
    isFetchingNextPage: false,
    isPending: false,
    onRetry: () => {},
    ref: noopRef,
  } satisfies BlogPostsPageViewProps,
};
