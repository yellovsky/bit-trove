...
import { useInfiniteShortArticlesQuery } from '@entities/articles';
...
export const SearchCommand = ({ open, onOpenChange, initialQuery = '' }: SearchCommandProps) => {
  ...
  const articlesQuery = useInfiniteShortArticlesQuery(
        ...
        <SearchInterface.Root>
          ...
          {!isLoading && !hasError && query.length >= 3 && totalResults > 0 && (
            <SearchInterface.Content>
              {/* Articles Section */}
              {articles.length > 0 &&
                articles.map((article) => <ArticleSearchResult article={article} key={article.id} />)}
            </SearchInterface.Content>
          )}
