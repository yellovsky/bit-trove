...
interface LocalizedShortShardModelData {
  id: string;
  slug: string;
...
export class LocalizedShortShardModel {
  static from(data: LocalizedShortShardModelData) {
    return new LocalizedShortShardModel(
      data.id,
      data.slug,
      data.publishedAt,
      data.title,
  ...
  filterPublished(): ResultOrExcluded<LocalizedShortShardModel> {
